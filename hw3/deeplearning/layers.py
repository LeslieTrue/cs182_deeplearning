...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (x, normalized_x, gamma, sample_mean, sample_var, eps)
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
        #############################################################################
        #                             END OF YOUR CODE                              #
...
def batchnorm_backward(dout, cache):
    ...
    pass
    N,D = dout.shape
    x, normalized_x, gamma, sample_mean, sample_var, eps = cache
    ...
    dvar = .5 * ivar * dsqrtvar

    dsqrt = 1. /N * np.ones((N,D)) * dvar

    dxmu_2 = 2 * xmu * dsqrt
    dx1 = dxmu + dxmu_2
    dmu = -1 * np.sum(dxmu+dxmu_2, axis=0)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(H_prime):
        for j in range(W_prime):
            ...
            # print(x_used)
            for f in range(F):
                out[:, f, i, j] = np.sum(x_used * w[f, :, :, :], axis=(1,2,3))
    for i in range(F):
        out[:, i, :, :] = out[:, i, :, :] + b[i]
    #############################################################################
    #                             END OF YOUR CODE                              #
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(H_prime):
        for j in range(W_prime):
            x_used = x[:, :, i*stride:i*stride + pool_height, j*stride:j*stride + pool_width]
            out[:, :, i, j] = np.max(x_used, axis=(2,3))
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(H_prime):
        for j in range(W_prime):
            x_used = x[:, :, i*stride:i*stride + pool_height, j*stride:j*stride + pool_width]# N, C, pool_height, pool_stride
            # print(x_used)
            after_pooling = np.max(x_used, axis=(2,3))
            # print(after_pooling[:,:,None,None])
            # print(after_pooling.size)
            # after_pooling.reshape(N, C, pool_height, pool_width)
            binary_result = (x_used == (after_pooling)[:,:,None,None])
            # print(binary_result)
            dx[:, :, i*stride:i*stride + pool_height, j*stride:j*stride + pool_width] += binary_result * (dout[:,:,i,j])[:,:,None,None]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    pass
    N, C, H, W = x.shape
    x = x.transpose(0,3,2,1).reshape(N*H*W, C) #0:1:2:3 N, C, H, W -> N,W,H,C
    out_temp, cache = batchnorm_forward(x,gamma,beta,bn_param)
    out = out_temp.reshape(N,H,W,C).transpose(0,3,2,1)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
