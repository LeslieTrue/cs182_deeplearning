...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for i in range(self.num_layers):
          if initialization == 'random':
            ...
            import math
            #############################################################################
            # TODO: Implement the he initization formula. Store the updated value in    #
            #############################################################################
            self.params['W%d' % (i+1)] = np.random.normal(loc=0.0, scale=1, size=(dims[i], dims[i+1])) * np.sqrt(2./dims[i])
            #############################################################################
            #                             END OF YOUR CODE                              #
            #############################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 2, -1, - 1):
            ...
            if self.use_batchnorm:
                dout, dw, db, dgamma, dbeta = affine_relu_bn_backward(dout, caches.pop())
