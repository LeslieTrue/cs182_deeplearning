...
class AttentionQKV(nn.Module):
    ...
    def forward(self, queries, keys, values, mask=None):
        ...
        if len(keys.shape) == 4:
            keys_t = th.transpose(keys, 2,3) #with head
        else:
            keys_t = th.transpose(keys, 1,2) #no head
        
        similarity = (queries @ keys_t)/th.sqrt(key_dim) # Compute the similarity according to the QKV formula
...
class MultiHeadProjection(nn.Module):
    ...
    def _split_heads(self, tensor):
        ...
        # so that each head acts independently (Hint: use the pytorch function "view")
        ...
        tensor = th.transpose(tensor, 1, 2)
        return tensor
        ##########################################################################################
